...
```{r}
dataframe <- read.csv("dataset/heart.csv", sep=",", row.names=NULL, header=TRUE)
dataframe <- dataframe[,1:13]
str(dataframe)
...
pp = preProcess(dataframe, method = c("center", "scale"))

#Aplicamos el escalado a train y test
datos <- predict(pp, dataframe)
```
