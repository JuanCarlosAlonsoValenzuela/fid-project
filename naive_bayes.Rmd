...
```{r}	
# model <- train(x_train, y_train, 'nb', trControl=trainControl(method="cv", number=10), laplace=10)
set.seed(123)
...
nb_grid <- expand.grid(
    laplace = c(0, 0.5, 1, 10, 20)),
    eps = c(0.01, 0.1, 0.3, 0.5, 1.0)
)
...
```{r}
cm <- table(test$target, y_pred_test)
cm
```
